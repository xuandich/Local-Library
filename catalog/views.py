...
def addbook(request):
    if request.method == 'POST':
        ...
        if title == '' or  publish_date == '' or authors_id_list == '':
            return HttpResponse('<h3 style="color: #c7254e">Không Được Để Trống</h3>')

        book_obj=Book.objects.create(title=title,publish_date=publish_date)
...
def editbook(request,id):
    ...
    if request.method == "POST":
        ...
        Book.objects.filter(pk=id).update(title=title, publish_date=publish_date)
        book_obj.authors.set(authors_id_list)
        return redirect("/book/")
